...
const Schedule = ({ clickableUser, setModal, timesheet, updateTimesheet }) => {
    ...
    const handleSave = async () => {
        try {
            const storedTimesheet = JSON.parse(sessionStorage.getItem('timesheetData') || '[]');
            const companyData = JSON.parse(sessionStorage.getItem("company") || '{}');
            const companyId = companyData.id;
    
            if (!companyId) {
                console.error("Missing companyId from sessionStorage. Please ensure it's set correctly.");
                return;
            }
            ...
            for (let entry of storedTimesheet) {
                ...
                const timesheetDocRef = doc(db, 'Company', companyId, 'Timesheet', entry.date);
                await setDoc(timesheetDocRef, { date: entry.date, shifts: entry.shifts }, { merge: true });
            }
    ...
    return (
        <DndProvider backend={HTML5Backend}>
                <div className='schedule-title-container'>
                    {period === 'Weekly' ? 
                        <button className='button-pill' onClick={() => handleToggleView()}>Daily View</button> : 
                        <button className='button-pill' onClick={() => handleToggleView()}>Weekly View</button>
                    }
                <h2 style={{ flex: 1, textAlign: 'center' }}>{period === 'Weekly' ? "Weekly Schedule" : "Daily Schedule"}</h2>
                <button className='button-pill' onClick={handleCancel}>Cancel</button>
                <button className='button-pill' onClick={handleSave}>Save</button>
                
            </div>
            <div className='schedule-container'>
            <AiOutlineCaretLeft className={period ==='Weekly'? 'arrow-icon':'arrow-icon-daily'} onClick={period === 'Weekly' ? handlePreviousWeek : handlePreviousDay}/>
                {period === 'Weekly' ?
