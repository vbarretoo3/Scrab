...
const Day = ({
  ...
  const [startDate, setStartDate] = useState(
    new Date(notFormattedDate.getTime())
  );
  ...
  const matchingDay =
    timesheet &&
    timesheet.find((entry) =>
      isSameDate(entry.date, getLocalDateAsString(notFormattedDate))
    );
  const shiftsForTheDay = matchingDay ? matchingDay.shifts : [];
  const effectiveShifts = shiftsForTheDay.length
    ? shiftsForTheDay
    : shifts || [];
  ...
  const generateShift = (date) => {
    ...
    return {
      ...
      Start: {
        seconds: Math.floor(
          convertLocalDateToUTC(newStartDate).getTime() / 1000
        ),
        nanoseconds: 0,
      },
      End: {
  ...
  const handleConfirm = () => {
    ...
    if (isChecked) {
      ...
      if (endDate > localMidnightAfterStartDate) {
        ...
        shiftsToAdd.push({
          ...
          Start: {
            seconds: Math.floor(
              convertLocalDateToUTC(startDate).getTime() / 1000
            ),
            nanoseconds: 0,
          },
          End: {
            seconds: Math.floor(
              convertLocalDateToUTC(shift1End).getTime() / 1000
            ),
            nanoseconds: 0,
          },
          Total:
            (Math.floor(shift1End.getTime() / 1000) -
        ...
        const shift2Start = new Date(
          localMidnightAfterStartDate.getTime() +
            localMidnightAfterStartDate.getTimezoneOffset() * 60000
        );
        shiftsToAdd.push({
          UserName: selectedUser.FirstName + " " + selectedUser.LastName,
          id: selectedUser.id,
          Start: {
            seconds: Math.floor(
              convertLocalDateToUTC(shift2Start).getTime() / 1000
            ),
            nanoseconds: 0,
          },
          End: {
            seconds: Math.floor(
              convertLocalDateToUTC(endDate).getTime() / 1000
            ),
            nanoseconds: 0,
          },
          Total:
            (Math.floor(endDate.getTime() / 1000) -
              Math.floor(shift2Start.getTime() / 1000)) /
            3600,
        });
      } else {
        shiftsToAdd.push({
    ...
    if (editingShift) {
      const matchingDateEntryForEditing = timesheet.find((entry) =>
        isSameDate(
          entry.date,
          getLocalDateAsString(
            new Date(editingShift.data?.Start?.seconds * 1000)
          )
        )
      );
      if (matchingDateEntryForEditing) {
        const idx = matchingDateEntryForEditing.shifts.findIndex(
          (s) =>
    ...
    for (let shift of shiftsToAdd) {
      const matchingDateEntry = timesheet.find((entry) =>
        isSameDate(
          entry.date,
          getLocalDateAsString(new Date(shift.Start.seconds * 1000))
        )
      );
      console.log(shift);
  ...
  return (
    <div
      ref={ref}
      className={period === "Weekly" ? "day-container" : "day-container-daily"}
    >
      <strong>{day}</strong>
      {effectiveShifts.map((shift, i) => (
        <div
          className={
            period === "Weekly" ? "schedule-shifts" : "schedule-shifts-daily"
          }
          key={i}
        ...
        >
          ...
          <br />
          End:{" "}
          {convertFirestoreTimestampToDate(shift.End).toLocaleTimeString(
            undefined,
            { hour: "2-digit", minute: "2-digit" }
          )}
        </div>
      ...
      >
          ...
          <>
            ...
            {isChecked === true && (
              <div className="week-repeat">
                <input
                  checked={repeatOnMon}
                  onChange={(e) => setRepeatOnMon(e.target.checked)}
                  type="checkbox"
                />
                <label>Monday</label>
                <input
                  checked={repeatOnTue}
                  onChange={(e) => setRepeatOnTue(e.target.checked)}
                  type="checkbox"
                />
                <label>Tuesday</label>
                <input
                  checked={repeatOnWed}
                  onChange={(e) => setRepeatOnWed(e.target.checked)}
                  type="checkbox"
                />
                <label>Wednesday</label>
                <input
                  checked={repeatOnThu}
                  onChange={(e) => setRepeatOnThu(e.target.checked)}
                  type="checkbox"
                />
                <label>Thursday</label>
                <input
                  checked={repeatOnFri}
                  onChange={(e) => setRepeatOnFri(e.target.checked)}
                  type="checkbox"
                />
                <label>Friday</label>
                <input
                  checked={repeatOnSat}
                  onChange={(e) => setRepeatOnSat(e.target.checked)}
                  type="checkbox"
                />
                <label>Saturday</label>
                <input
                  checked={repeatOnSun}
                  onChange={(e) => setRepeatOnSun(e.target.checked)}
                  type="checkbox"
                />
                <label>Sunday</label>
                <input
                  checked={repeatOnWek}
                  onChange={(e) => setRepeatOnWek(e.target.checked)}
                  type="checkbox"
                />
        ...
        <div className="schedule-modal-buttons">
          <button
            className="schedule-modal-button-confirm"
            onClick={handleConfirm}
          >
            Confirm
          </button>
          <button
            className="schedule-modal-button-cancel"
          ...
          {editingShift !== null && (
            <button
              className="schedule-modal-button-delete"
